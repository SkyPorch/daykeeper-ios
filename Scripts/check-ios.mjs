...
// The fixture is a loopback HTTP server that needs nothing from the simulator,
// so it starts first, while the machine is still idle. Booting a cold simulator
// can saturate a shared runner for minutes -- one CI boot spent 3m49s in data
// migration -- and a fixture competing with that for CPU has timed out before it
// could print its readiness line. Ordering it first removes the contention
// instead of widening the deadline to hide it.
//
// The invariant this replaces still holds: xcodebuild starts only after
// bootstatus returns for this exact owned device, so a cold boot can never eat
// into the SDK's intentionally short request deadline.
